...
export default function gallery(options: PluginOptions): Plugin {
    ...
    function generateManifestModule() {
        ...
        return `// Generated by the gallery plugin:
const manifest = {
    data: new Uint8Array([${bytes.join(",")}]),
    schema: {
        image: '${resolveSchema('image')}',
        thumb: '${resolveSchema('thumb')}',
    },
    tagLoca: {
${[...languages].map(l => `        ${l}: '${locaName(l)}',\n`).join('')}    },
};
export default manifest;`;
    }

    function generateManifest(force: boolean = false): Manifest {
        ...
        const photos: Record<string, PhotoInfo> = [...input.all()].reduce((r, p) => {
            if (force || (p.image.url !== undefined && p.thumb.url !== undefined)) {
